...
const facts = [
  { label: "Classes 9–12", detail: "Boards-aligned, subject by subject" },
  { label: "Video + notes + tests", detail: "One login instead of three subscriptions" },
  { label: "AI-analysed results", detail: "Every test shows exactly what to revise next" },
];
          ...
          <dl className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-3">
            {facts.map((fact) => (
              <div key={fact.label} className="border-t border-line pt-4">
                <dt className="font-display text-sm font-bold text-ink">{fact.label}</dt>
                <dd className="mt-1 text-xs leading-5 text-muted">{fact.detail}</dd>
              </div>
            ))}
          </dl>
        </Reveal>
